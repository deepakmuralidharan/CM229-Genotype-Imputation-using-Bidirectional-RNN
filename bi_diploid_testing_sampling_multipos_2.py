...
pred_vector = tf.concat(0, pred_vector)
...
with tf.Session() as sess:
    ...
    for individual in range(0,n_test):
        ...
        for position in xrange(0,total_pos):
            row_test_input[pos_arr[position]]= np.random.randint(0,2)
        ...
        count_matrix = np.zeros((total_pos, n_classes),dtype = float)

        for it in xrange(0,max_iter + burn_in_period):
            ...
            for position in xrange(0,total_pos):
                ...
                y_pred_vector = sess.run(pred_vector, feed_dict={x: x_b,
                                                istate_fw: np.zeros((1, 2*n_hidden)),
                                                istate_bw: np.zeros((1, 2*n_hidden))})
                y_pred_vector = np.asarray(y_pred_vector) # (50,3)
                row_test_input[pos_arr[position]]= np.random.choice([0,1,2], p = y_pred_vector[pos_arr[position],:])

                if it > burn_in_period:
                    count_matrix[int(position), int(row_test_input[pos_arr[position]])] += 1

        print count_matrix
        values = np.argmax(count_matrix, 1)

        for position in xrange(0,total_pos):
            row_test_input[pos_arr[position]] = values[position]
        ...
        print 'Mismatches for individual {}: {}'.format(individual, mismatches[0])
