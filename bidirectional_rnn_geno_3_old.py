...
max_epochs = 10# maximum number of epochs we want the training to run fo10
...
data = np.loadtxt('/Users/deepakmuralidharan/Documents/Bidirectional-LSTM/data/geno_loc_2.txt',delimiter=',')
...
with tf.Session() as sess:
    ...
    for i in range(0, n_test):
        ...
        for new_value in (0,1):
            ...
            #print 'Predicted for new_value {} : {}'.format(new_value,y_pred)
            #print 'Ground_truth: {}'.format(test_label)
            loss_arr.append(y_pred[0,pos-1])

        loss_arr = np.asarray(loss_arr)

        print 'Loss-Arr: {}'.format(sum(loss_arr))
