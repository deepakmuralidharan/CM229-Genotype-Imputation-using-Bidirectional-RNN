...
# Network Parameters
n_input = 3 # dimension of input data (for genotype data-- since value is binary n_input = 1)
n_steps = 50 # number of columns in the data matrix
n_hidden = 10 # hidden layer -- hyperparameter -- values can range between 1-10
n_classes = 3 # dimension of outout (for genotype data modelling - output is 1 or 0)
...
_y  = tf.squeeze(y,[2])
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(pred, _y)) # Softmax loss
...
with tf.Session() as sess:
    ...
    for epoch in xrange(max_epochs):
        ...
        if training_loss < best_train_epoch:
            saver.save(sess, './geno_uni.weights')
